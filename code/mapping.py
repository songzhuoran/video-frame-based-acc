...
def bframe_gen_kernel(fcnt):
    ...
    if ('%08d' % int(row[0])) not in os.listdir(B_OUT_DIR+classname):
        os.mkdir(B_OUT_DIR+classname +'/'+'%08d' % int(row[0]))

    tmp_list=[]
    print(len(tmp_class_list))
    print(B_OUT_DIR+classname + '/' +'%08d' % int(row[0]))
    for i in tmp_class_list:
        
        cv2.imwrite(B_OUT_DIR+classname + '/'+'%08d' % int(row[0])+'/'+str(i)+'.png',bframe_img[i])
        tmp_list.append(Image_mat(i,bframe_img[i]))
        
    frame_mat[B_OUT_DIR+classname +'/'+'%08d' % int(row[0])] = tmp_list
